...
import { auth, db, googleProvider, githubProvider } from '../lib/AuthContext'

// Re-export providers from firebase.js
import { googleProvider as gp, githubProvider as ghp } from '../lib/firebase'
...
export default function RoleSelect() {
    ...
    try {
      if (mode === 'signup') {
        ...
        await setDoc(doc(db, 'profiles', cred.user.uid), {
          email, name, role,
          phone: '', photoURL: '', provider: 'password',
          created_at: new Date().toISOString()
        })
        await sendEmailVerification(cred.user)
        await auth.signOut()
        setInfo('✅ Verification email sent! Please check your inbox, verify your email, then sign in.')
      } else {
  ...
  const handleGithub = async () => {
    ...
    try {
      localStorage.setItem('phx_pending_role', JSON.stringify({ role }))
      await signInWithPopup(auth, ghp)
      navigate('/')
    } catch (err) {
      setError(friendlyError(err.code))
    }
    ...
    <div style={{ minHeight: '100vh', background: '#0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ width: '100%', maxWidth: 440, background: '#1e293b', borderRadius: 16, padding: '2rem', border: '1px solid #334155' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
          ...
          <h1 style={{ color: '#06b6d4', fontSize: 22, fontWeight: 700 }}>PHX-MASTER</h1>
          <p style={{ color: '#94a3b8', fontSize: 13 }}>Virtual Physics Laboratory</p>
        </div>
        ...
        <div style={{ display: 'flex', background: '#0f172a', borderRadius: 8, padding: 4, marginBottom: '1.5rem' }}>
          {['student', 'teacher'].map(r => (
            <button key={r} onClick={() => setRole(r)}
              style={{ flex: 1, padding: '8px', borderRadius: 6, border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: 13, textTransform: 'capitalize',
                background: role === r ? '#06b6d4' : 'transparent', color: role === r ? '#fff' : '#94a3b8' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: '1.5rem' }}>
          {['login', 'signup', 'phone'].map(m => (
            <button key={m} onClick={() => { setMode(m); clearMessages() }}
              style={{ flex: 1, padding: '7px', borderRadius: 6, border: `1px solid ${mode === m ? '#06b6d4' : '#334155'}`, cursor: 'pointer', fontSize: 12, fontWeight: 600,
                background: mode === m ? '#164e63' : 'transparent', color: mode === m ? '#06b6d4' : '#64748b' }}>
        ...
        {(mode === 'login' || mode === 'signup') && (
          <form onSubmit={handleEmailAuth}>
            ...
            {mode === 'signup' && (
              <p style={{ color: '#94a3b8', fontSize: 11, marginTop: 6, textAlign: 'center' }}>
          ...
          <form onSubmit={otpSent ? verifyOTP : sendOTP}>
            {!otpSent ? (
              <input value={phone} onChange={e => setPhone(e.target.value)}
                placeholder="Phone number: +91XXXXXXXXXX" required style={inp} />
            ) : (
            ...
            {otpSent && (
              <button type="button" onClick={() => { setOtpSent(false); setOtp(''); window.recaptchaVerifier = null }}
        ...
        <p style={{ textAlign: 'center', color: '#64748b', fontSize: 12, marginTop: '1.2rem' }}>
          ...
          <span onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); clearMessages() }}
...
const inp = {
  ...
  borderRadius: 8, color: '#f1f5f9', fontSize: 14, marginBottom: 10, outline: 'none',
